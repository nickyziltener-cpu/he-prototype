...
(function () {
  ...
  function render() {
    ...
    if (pill) {
      pill.textContent = count === 0 ? 'Speichere alles, was interessant aussieht' : count + ' gespeichert';
    }

    // ── Header badge (overview + category screens) ────────────────────────────
    var badge = document.querySelector('[data-header-counter]');
    if (badge) {
      badge.textContent = count === 0 ? '0 gespeichert' : count + ' gespeichert';
    ...
    if (trayLeft) {
      if (count === 0) {
        trayLeft.innerHTML = '<span class="tray-empty-label">Speichere alles, was dir auffällt</span>';
      } else {
        trayLeft.innerHTML = '<span class="tray-count-label">' + count + ' gespeichert</span>';
      }
    }
    ...
    document.querySelectorAll('[data-treatment-id]').forEach(function (btn) {
      ...
      if (selected) {
        ...
        btn.innerHTML = SVG_CHECK + ' Gespeichert';
      } else {
        btn.classList.remove('added');
        btn.innerHTML = SVG_HEART + ' Möchte ich ausprobieren';
      }
    });
  }
